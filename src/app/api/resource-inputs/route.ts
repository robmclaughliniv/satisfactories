...
// POST /api/resource-inputs - Create a new resource input
export async function POST(request: NextRequest) {
  try {
    ...
    const resourceInput = await prisma.resourceInput.create({
      data: {
        factoryId,
        name: name || null,
    ...
    return NextResponse.json(resourceInput, { status: 201 });
  } catch (error) {
